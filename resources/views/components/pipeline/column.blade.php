...
@php
    $stageDeals = collect($deals)->where('stage_id', $stage['id']);
    ...
    $dotColor = $colors[$stage['id']] ?? 'bg-slate-300';
@endphp

<div
    class="flex-shrink-0 w-80 flex flex-col h-full rounded-xl bg-slate-50/50 border border-slate-200/60"
    @dragover.prevent="dragOver($event, {{ $stage['index'] }})"
    @drop="drop($event, '{{ $stage['id'] }}', {{ $stage['index'] }})"
    :class="{ 'bg-red-50/50 cursor-not-allowed opacity-60': isInvalidDrop({{ $stage['index'] }}) }"
>
    ...
    <div class="p-3 space-y-3 flex-1 overflow-y-auto min-h-[500px]">
        ...
        <button class="w-full py-3 border-2 border-dashed border-slate-200 rounded-xl text-slate-400 hover:border-emerald-400 hover:text-emerald-500 hover:bg-emerald-50 transition-all flex items-center justify-center gap-2 group">
